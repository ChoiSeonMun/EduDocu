...
```
라고 정의되어 있다. 그럼 프로그램에서 변수란 무슨 뜻일까?

우리는 컴퓨터를 사용할 때 여러가지 데이터를 읽거나 쓴다. 그럼 이 데이터는 어디에 쓰이는 것이며, 어디서부터 불러오는 것일까? 바로 메모리이다. **변수**(variable)는 이 메모리에 이름을 붙여주는 것과 같다. 혹, **객체**(object)라고도 한다. 우리는 이 변수를 통해 메모리의 주소를 직접 외우지 않아도 사용할 수 있게 된다. 상술한 뜻을 통해 유추할 수 있듯, 변수 안에 들어가는 값은 가변성을 지닌다. 변수는 이렇게 선언 및 정의한다.
...
좋은 식별자를 쓰려면 다룰 자료와 관련된 유의미한 이름을 지어주는 것이 좋다. 가령 `int age`같은 것들이다. 그렇다보니 식별자의 길이가 길어지는 일이 허다한 데, 단어를 구분하기 위해 여러가지 방법들이 있다. 스네이크 케이스(snake_case), 카멜 케이스(camelCase), 파스칼 케이스(PascalCase) 등이다.

#### - 상수

**상수**(constant)는 변수와 반대되는 변하지 않는 값을 일컫는다. 변수 선언시 타입 앞쪽 혹은 뒤쪽에 `const` 키워드를 붙여주면 된다. 예를 들면 `const int a`, `int const b`이다. 상수는 꼭 <b>초기화</b>를 해야한다. 그렇지 않으면 컴파일러 오류가 난다.

- 리터럴과 기호상수

**리터럴**(literal)이란, 고정된 값을 의미한다. `100` `1.54` 등과 같은 것들이다. 이 리터럴을 그냥 사용한다면 그 당시는 알 수 있을지 모르나, 다시 봤을 땐 모를 수도 있고, 다른 사람은 유추하기 힘들 수도 있다. 사람은 숫자보다 문자, 기호에 능하기 때문에 어떤 기호를 붙여주는 것이 가독성도 높고 의미를 유추하기에도 편할 것이다. 그 방법은 `#define`을 통해서 할 수 있다. 사용 방법은 다음과 같다.
...
**유효범위**(scope)는 변수가 유효한 범위를 말한다. 즉, 사용할 수 있는 범위를 얘기한다. 사실 여러가지가 있지만, 크게 두 가지로 살펴보려 한다.

> - 전역 범위
>
> 어떤 변수가 전역 범위를 가진다는 것은 모든 곳에서 그 변수를 볼 수 있다<small>(사용할 수 있다)</small>는 것을 의미한다. 다음 예제를 보자.

```c++
//전역 변수 예제
int gGlobalVariable; // 자동으로 0으로 초기화 된다.

int main(void)
{
  gGlobalVariable = 2; // 2를 대입

  return 0;
}
```

> 전역 변수는 다른 파일에서까지 볼 수 있다. 다른 파일에서 `extern` 키워드를 붙여 변수를 '선언'하면 쓸 수 있다.
>
> - 지역 범위
> 어떤 변수가 지역 범위를 가진다는 것은 그 지역에서만 변수를 볼 수 있다는 것을 의미한다. 다음 예제를 보자.

```c++
//지역 변수 예제
int main(void)
{
  int localVariable;    // 지역 변수는 미초기화시 쓰레기 값을 가짐
  {                     //이런 것을 블록(block)이라고 한다.
    int localVariable;  // 이런 것을 '가리우기(shadowing)'이라고 한다.
  }
...
> 지역 범위를 가지는 변수는 그 지역을 벗어나는 순간 소멸된다.
<!--- static 키워드, 전역 변수 가리우기 --->
...
<center>
<img src="http://i.imgur.com/UeCu3cz.png">
</center>
하나씩 살펴보자.
...
> - **비교/관계 연산자**
>
>비교/관계 연산자는 비교 관계를 따지는 연산자이다. 피연산자가 두 개 필요하다. `>`, `>=`, `<`, `<=`, `==`, `!=`가 있다. `bool` 값을 반환한다.
>
> - **논리 연산자**
>
>논리 연산자는 `&&`, `||`, `!`로 나뉜다. <small> (논리적 NOT 연산자를 제외하고) </small> 피연산자가 두 개 필요하며, `bool` 값을 반환한다. 논리적 합과 곱 연산자는 단축 평가를 하게 된다.
> <!--- 단축평가란? --->
>
> - **비트 연산자**
>
>비트 수준을 다루게 해주는 연산자이다. `<<`, `>>`와 같은 시프트(shift) 연산과 `&`, `|`, `^(xor)`, `~(not)` 연산자로 이루어진다. <small> (비트 NOT 연산자를 제외하고) </small> 모두 피연산자가 두 개 필요하다. 부호 없는 정수형에 많이 이용된다.
>
> - **복합 대입 연산자**
>
...
1. 지수와 가수 : http://prof.dongju.ac.kr/syhong/public_html/teaching/c/c03.htm
